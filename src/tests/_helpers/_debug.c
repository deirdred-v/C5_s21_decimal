...
void s21_decimal_to_string(s21_decimal decimal, char *res) {
  res[0] = '\0';
  // Не используем s21_is_correct_decimal(), т.к. в тестируемой библиотеке её
  // может не быть
...
void s21_format_decimal_to_str(s21_decimal decimal, char *res) {
  char *str;
  // Не используем s21_decimal_get_power(), т.к. в тестируемой библиотеке её
  // может не быть
  int power = test_decimal_get_power(decimal);
  // Не используем s21_decimal_get_sign(), т.к. в тестируемой библиотеке её
  // может не быть
  ...
  char *ptr = res;
  if (sign == 1 &&
      (decimal.bits[0] != 0 || decimal.bits[1] != 0 || decimal.bits[2] != 0)) {
    *(ptr++) = '-';
  }
